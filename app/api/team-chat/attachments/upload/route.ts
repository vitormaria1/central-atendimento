import { NextResponse } from "next/server";
import { getSession } from "@/lib/auth";
import { withApi } from "@/lib/api";
import { dbQuery } from "@/lib/db";

export const dynamic = "force-dynamic";
export const runtime = "nodejs";

const MAX_FILE_BYTES = 10 * 1024 * 1024; // 10MB
const MAX_FILES_PER_REQUEST = 5;

function safeFilename(name: string) {
  const base = name.replace(/[^\w.\-() ]+/g, "_").slice(0, 120).trim();
  return base || "arquivo";
}

export const POST = withApi(async (req: Request) => {
  const session = await getSession();
  if (!session) return NextResponse.json({ error: "Unauthorized" }, { status: 401 });

  const form = await req.formData().catch(() => null);
  if (!form) return NextResponse.json({ error: "Invalid form-data" }, { status: 400 });

  const messageIdRaw = String(form.get("messageId") ?? "").trim();
  const messageId = Number.parseInt(messageIdRaw, 10);
  if (!Number.isFinite(messageId)) return NextResponse.json({ error: "Invalid messageId" }, { status: 400 });

  const files = form.getAll("files").filter((v): v is File => v instanceof File);
  if (files.length === 0) return NextResponse.json({ error: "Nenhum arquivo" }, { status: 400 });
  if (files.length > MAX_FILES_PER_REQUEST) return NextResponse.json({ error: "Muitos arquivos" }, { status: 400 });

  // ensure message exists (and belongs to channel; internal chat only, so just existence)
  const { rowCount } = await dbQuery("select 1 from team_chat_messages where id = $1", [messageId]);
  if (!rowCount) return NextResponse.json({ error: "Mensagem não encontrada" }, { status: 404 });

  const created: Array<{ id: string; filename: string; mimetype: string | null; sizeBytes: number }> = [];

  for (const file of files) {
    if (file.size > MAX_FILE_BYTES) return NextResponse.json({ error: `Arquivo muito grande: ${file.name}` }, { status: 400 });
    const buf = Buffer.from(await file.arrayBuffer());
    const filename = safeFilename(file.name);
    const mimetype = file.type || null;
    const sizeBytes = buf.byteLength;

    const { rows } = await dbQuery<{ id: string }>(
      `
        insert into team_chat_attachments (message_id, filename, mimetype, size_bytes, content)
        values ($1, $2, $3, $4, $5)
        returning id::text
      `,
      [messageId, filename, mimetype, sizeBytes, buf],
    );
    if (rows[0]?.id) created.push({ id: rows[0].id, filename, mimetype, sizeBytes });
  }

  return NextResponse.json({ items: created });
});
