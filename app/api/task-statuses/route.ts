...
export const GET = withApi(async () => {
  ...
  const { rows } = await dbQuery<{ id: string; name: string; color: string; sort_order: number }>(
    `select id, name, color, sort_order from task_status_meta order by sort_order asc, id asc`,
    [],
  );
...
export const POST = withApi(async (req: Request) => {
  ...
  // Ensure enum value exists (so tasks can store it)
  await dbQuery(`alter type task_status add value if not exists '${id}'`, []);

  const { rows } = await dbQuery<{ id: string; name: string; color: string; sort_order: number }>(
    `
      insert into task_status_meta (id, name, color, sort_order)
      values ($1, $2, $3, (select coalesce(max(sort_order), 0) + 10 from task_status_meta))
  ...
  const row = rows[0];
  if (!row) return NextResponse.json({ error: "Falha ao criar status" }, { status: 500 });
...
export const PATCH = withApi(async (req: Request) => {
  ...
  await dbQuery(`update task_status_meta set ${sets.join(", ")} where id = $${params.length}`, params);
